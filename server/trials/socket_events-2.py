...
async def setup_socket_events(state_manager: StateManager, whisper_model, fb_model=None, fb_tokenizer=None):
    """
    ...
    @socketio.on("state-change-by-react")
    async def on_state_change_r(data):
        if data["target"] == "fastapi":
            new_state = data.get("state", "idle")
            await state_manager.set_state(new_state)

    @socketio.on("state-change-by-node")
    async def on_state_change_n(data):
        if data["target"] == "fastapi":
            new_state = data.get("state", "idle")
            await state_manager.set_state(new_state)
    ...
    async def handle_audio_chunk(data):
        ...
        if current_state not in ["running", "resumed"]:
            print("Probably you stopped or resumed the api")
            return  # Ignore chunks if not in the correct state
        global warning_sent
        try:
...
async def shutdown_processes(temp_audio_folder, state_manager):
    """
    ...
    try:
        ...
        # Stop ongoing tasks if the current state is "running"
        current_state = await state_manager.get_state()
        if current_state == "running":
            print("Stopping ongoing processes...")
            # Signal ongoing tasks to stop
